...
type PurchaseItemType = { info: ProductInfo; amount: number; id: string }
type ProductStore = { increaseAmount: (id: string) => void; decreaseAmount: (id: string) => void; remove: (id: string) => void }
  ...
  return (
    <div className="flex items-center justify-between border rounded-md py-4">
      ...
      <div className="flex flex-row gap-2 items-center mr-3">
        <div className="text-sm mr-2">×{purchaseItem.amount}</div>
        <div className="flex flex-row border rounded-md justify-between items-center">
          <button type="button" className="size-8 rounded-full" onClick={() => store.decreaseAmount(purchaseItem.id)}>
            <Minus className="size-4 mx-auto" />
          </button>
          <Separator orientation="vertical" className="h-4" />
          <button type="button" className="size-8 rounded-full" onClick={() => store.increaseAmount(purchaseItem.id)}>
